...
if len(sys.argv) < 2:
	print('\nRun: ' + text.GREEN + 'python chameleon_circuit_sequence.py ' + text.RED + text.BOLD + 'SIMULATION_NAME\n' + text.END)
	exit()
...
try:
	...
	with open(console[0], 'r') as file:
		...
		L = (extract_brackets(lines[0]).split(sep = ','))
		T = (extract_brackets(lines[1]).split(sep = ','))
		C = extract_brackets(lines[2]).split(sep = ',')
		V = (extract_brackets(lines[3]).split(sep = ','))
except:
